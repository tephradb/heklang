...
The formatter changed what a mistake here costs. `bindings/rust/build.rs` compiles
`src/parser.c` into this crate and so into the `hek` binary, so this is a build input rather
than an editor convenience, and the "regenerate and commit `src/`" rule below is now
load-bearing: skip it and `hek fmt` parses a language nobody is writing.
`../cli/tests/grammar.rs` is the standing check that the grammar and the acceptance sources
in `../hek` still agree, which until now was a command someone had to remember to run.
